...
RELEASE?=$(shell jq -r .tag_name latest.json 2> /dev/null)

TARBALL:=${DOWNLOADS_DIR}/zed-linux-${ARCH}-${RELEASE}.tar.gz
...
download: ${TARBALL}
unpack: ${RELEASE}/zed.app
link: $(LOCAL_BIN)/zed

${TARBALL}:
	@test -n "${RELEASE}" || (echo "RELEASE is empty. run 'make refresh' to fetch releases" && exit 1)
	mkdir -p ${DOWNLOADS_DIR}
	wget https://github.com/zed-industries/zed/releases/download/${RELEASE}/zed-linux-${ARCH}.tar.gz -O $@.tmp || (rm -f $@.tmp && exit 1)
	mv $@.tmp $@

zed.app: ${RELEASE}/zed.app
	ln -fs ${RELEASE}/zed.app zed.app

${RELEASE}/zed.app: ${TARBALL}
	mkdir -p ${RELEASE}
	touch ${RELEASE}/.nobackup
	tar -C ${RELEASE} -xf ${TARBALL}
